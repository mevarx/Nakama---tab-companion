...
const CHARACTERS = [
  { id: 'tanjiro',  label: 'TANJIRO',  theme: 'default',       emoji: '🔥', colors: { hoodie:'#7A2828', secondary:'#1C1917', skin:'#FDE047', hair:'#334155', accent:'#10B981', primary:'#4F46E5' }},
  { id: 'itadori',  label: 'ITADORI',  theme: 'claude',        emoji: '⚡', colors: { hoodie:'#F472B6', secondary:'#EF4444', skin:'#FDE047', hair:'#3B2E2A', accent:'#F5E6D3', primary:'#CC785C' }},
  { id: 'toji',     label: 'TOJI',     theme: 'chatgpt',       emoji: '🗡️', colors: { hoodie:'#111827', secondary:'#111827', skin:'#FDE047', hair:'#0F172A', accent:'#ECECF1', primary:'#10A37F' }},
  { id: 'gojo',     label: 'GOJO',     theme: 'gemini',        emoji: '🌀', colors: { hoodie:'#4285F4', secondary:'#0F172A', skin:'#FDE047', hair:'#0F172A', accent:'#E8F0FE', primary:'#1A73E8' }},
  { id: 'sasuke',   label: 'SASUKE',   theme: 'youtube',       emoji: '🌩', colors: { hoodie:'#1E1B4B', secondary:'#F1F5F9', skin:'#FDE047', hair:'#0F0F0F', accent:'#FEEBEB', primary:'#FF0000' }},
  { id: 'aizen',    label: 'AIZEN',    theme: 'google',        emoji: '🎭', colors: { hoodie:'#F8FAFC', secondary:'#0F172A', skin:'#FDE047', hair:'#5C3A21', accent:'#FBBC05', primary:'#4285F4' }},
  { id: 'kira',     label: 'KIRA',     theme: 'mail',          emoji: '📓', colors: { hoodie:'#D6D3D1', secondary:'#DC2626', skin:'#FDE047', hair:'#4A3B32', accent:'#EAEAEA', primary:'#EA4335' }},
  { id: 'luffy',    label: 'LUFFY',    theme: 'reddit',        emoji: '⚓', colors: { hoodie:'#EF4444', secondary:'#1D4ED8', skin:'#FDE047', hair:'#2D3748', accent:'#FFEBE5', primary:'#FF4500' }},
  { id: 'ace',      label: 'ACE',      theme: 'linkedin',      emoji: '🔥', colors: { hoodie:'#F97316', secondary:'#EF4444', skin:'#FDE047', hair:'#0F172A', accent:'#E9E5DF', primary:'#0A66C2' }},
];
...
function broadcastSettings(state) {
  ...
  if (!ext?.tabs) return;
  const query = ext.tabs.query({});
  const handler = (tabs) => {
    if (!tabs) return;
    tabs.forEach(tab => ext.tabs.sendMessage(tab.id, { type: 'UPDATE_SETTINGS', state }).catch(() => {}));
  };
  if (query instanceof Promise) query.then(handler).catch(() => {});
...
function drawCharacterMini(ctx, size, charId, frame = 0) {
  ...
  const ox = 8, bobY = (frame === 2 || frame === 3) ? 1 : 0;
  const oy = 8 + bobY;
  const eyeState = frame === 3 ? 'blink' : 'open';
  ...
  drawCharacterStyle(ctx, charId, ox, oy, c, frame);
...
function drawCharacterStyle(ctx, style, ox, oy, c, frame) {
  if (style === 'tanjiro') {
    ...
    for (let cy = 0; cy < 9; cy+=3) for (let cx = 0; cx < 14; cx+=3) if (Math.floor((cx/3)+(cy/3))%2===0) ctx.fillRect(ox+1+cx, oy+10+cy, 3, 3);
...
function renderOverview(state) {
  ...
  const xpNext = LEVEL_THRESHOLDS[lvl + 1] !== undefined ? LEVEL_THRESHOLDS[lvl + 1] : 8000;
  ...
  const totalVisits = Object.values(state.siteVisits || {}).reduce((a, b) => a + b, 0);
  el('stat-sites').textContent = totalVisits;
  ...
  if (pending.length === 0 && done.length === 0) {
    questList.innerHTML = '<div class="empty-state">No quests found</div>';
    return;
  }
...
function renderCharacterPanel(state) {
  ...
  CHARACTERS.forEach(char => {
    ...
    const ctx = canvas.getContext('2d');
    drawCharacterMini(ctx, 48, char.id, 0);
    ...
    card.addEventListener('click', async () => {
      if (mode !== 'manual') {
        showToast('Switch to MANUAL mode first', 'info');
        return;
      }
  ...
  updateCurrentChar(current, state);
  currentCharId = current.id;
  startCharPreviewAnimation(current.id);
}

function updateCurrentChar(char, state) {
...
function renderSidebarStatus(state) {
  ...
  label.textContent = enabled ? 'COMPANION ACTIVE' : 'COMPANION OFFLINE';
...
function renderSiteStats(state) {
  ...
  SITE_KEYS.forEach(key => {
    ...
    const time    = state.siteTime?.[key] || 0;
      ...
      <div class="site-stat-card">
        ...
        <div class="site-metrics">
          <div class="site-metric">
            <div class="metric-lbl">VISITS</div>
            <div class="metric-val">${visits}</div>
          </div>
          <div class="site-metric">
            <div class="metric-lbl">ITEMS</div>
            <div class="metric-val">${items}</div>
          </div>
          <div class="site-metric">
            <div class="metric-lbl">TIME</div>
            <div class="metric-val">${mins}m</div>
          </div>
        </div>
      </div>`);
  });
...
document.getElementById('mode-manual').addEventListener('click', async () => {
  ...
  broadcastSettings(state);
  renderCharacterPanel(state);
  showToast('Character mode: MANUAL — pick a character below', 'info');
});

// Quick actions
document.getElementById('btn-force-react').addEventListener('click', async () => {
  const ext = typeof browser !== 'undefined' ? browser : (typeof chrome !== 'undefined' ? chrome : null);
  if (!ext?.tabs) { showToast('Extension API not available', 'error'); return; }
  const send = (tabs) => tabs.forEach(tab => ext.tabs.sendMessage(tab.id, { type: 'FORCE_STATE', state: 'react', duration: 2000 }).catch(() => {}));
  const q = ext.tabs.query({ active: true });
  if (q instanceof Promise) q.then(send).catch(() => {});
  else ext.tabs.query({ active: true }, send);
  showToast('Forced REACT animation!', 'success');
});

document.getElementById('btn-force-special').addEventListener('click', async () => {
  const ext = typeof browser !== 'undefined' ? browser : (typeof chrome !== 'undefined' ? chrome : null);
  if (!ext?.tabs) { showToast('Extension API not available', 'error'); return; }
  const send = (tabs) => tabs.forEach(tab => ext.tabs.sendMessage(tab.id, { type: 'FORCE_STATE', state: 'special', duration: 3000 }).catch(() => {}));
  const q = ext.tabs.query({ active: true });
  if (q instanceof Promise) q.then(send).catch(() => {});
  else ext.tabs.query({ active: true }, send);
...
document.getElementById('btn-spawn-item').addEventListener('click', async () => {
  const ext = typeof browser !== 'undefined' ? browser : (typeof chrome !== 'undefined' ? chrome : null);
  if (!ext?.tabs) { showToast('Extension API not available', 'error'); return; }
  const send = (tabs) => tabs.forEach(tab => ext.tabs.sendMessage(tab.id, { type: 'SPAWN_ITEM' }).catch(() => {}));
  const q = ext.tabs.query({ active: true });
  if (q instanceof Promise) q.then(send).catch(() => {});
  else ext.tabs.query({ active: true }, send);
...
document.getElementById('btn-add-xp').addEventListener('click', async () => {
  ...
  while (targetLevel < 8) {
    const req = LEVEL_THRESHOLDS[targetLevel + 1];
    if (req !== undefined && state.xp >= req) targetLevel++;
    else break;
  }
  const leveled = targetLevel > state.level;
